...
public class SampleExperiment {
    ...
    private void saveResultsToCSVFile(Vector< pair<Integer,evaluationPoint> > results, String fileName) {
        try {
            FileWriter FW = new FileWriter(new File(fileName));
            FW.write("#Results from SampleExperiment.java.  First line is means, second line is standard deviations.\n");
            for (pair point : results) {
                FW.write(point.getl() + ",");
		evaluationPoint p = (evaluationPoint)point.getr();
		FW.write(""+p.mean + ",");
		FW.write(""+ p.standardDeviation + ",");
        FW.write(""+ p.numofsteps);
		FW.write("\n");
            }
	...
	public double numofsteps;

        public evaluationPoint(double mean, double standardDeviation, double steps) {
	...
	int sum_of_steps = 0;
        ...
        for (i = 0; i < n; i++) {
            ...
            //System.out.println("The return is " + this_return);//********************************** helper statement by user
        ...
        mean = sum / (double)n;
	double avg_steps = sum_of_steps / (double)n;
        variance = (sum_of_squares - (double)n * mean * mean) / ((double)n - 1.0f);
        return new evaluationPoint(mean, Math.sqrt(variance), avg_steps);
    }
    /*
    ...
    void offlineDemo() {
        Vector< pair<Integer,evaluationPoint> > results = new Vector< pair<Integer,evaluationPoint> >();
        evaluationPoint initialScore = evaluateAgent();
	    pair<Integer,evaluationPoint> r = new pair<Integer,evaluationPoint>(0, initialScore);
        results.add(r);
	    ...
	    pair<Integer,evaluationPoint> p = new pair<Integer,evaluationPoint>(episode, currentScore);
            results.add(p);
        }
    ...
    public void runExperiment() {
        ...
        System.out.println("After Episode\tMean Return\tStandard Deviation\tNum of Steps\n-------------------------------------------------------------------------");
        offlineDemo();
        ...
        RLGlue.RL_env_message("set-start-state 1 10");
